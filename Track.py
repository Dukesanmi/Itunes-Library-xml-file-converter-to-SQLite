...
#key>Name</key><string>24K Magic</string>
#<key>Artist</key><string>Bruno Mars</string>
...
for entry in all:
	...
	cur.execute(' SELECT id FROM Artist WHERE name=?',(artist,)) #to get the primary key of the row
	...
	cur.execute(''' INSERT OR REPLACE INTO Track(title, len, count, rating, album_id, genre_id) VALUES(?,?,?,?,?,?)''',(name, length, count, rating, album_id, genre_id))
